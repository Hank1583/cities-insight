...
export default function CityClient({ cityCode }: { cityCode: string }) {
    ...
    <div className="max-w-7xl space-y-6">
      ...
      {loading && (
        <div className="space-y-4">
          <div className="rounded-2xl p-6 h-40 animate-pulse bg-slate-200" />
          <div className="grid grid-cols-4 gap-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="rounded-xl border border-slate-200 p-4 h-20 animate-pulse bg-slate-100" />
            ))}
          </div>
        </div>
      )}
