import { Wind, Thermometer, CloudRain, Droplets, Zap, Activity, AlertTriangle, BatteryCharging } from 'lucide-react'
import { CitySummary } from '@/types/city'

interface Props { metrics: CitySummary['metrics'] & { electricity_monthly?: number } }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 gap-3">
        ...
        <MetricBadge label="月用電量" value={metrics.electricity_monthly ?? 0} unit="億度" icon={BatteryCharging} color="bg-orange-50 text-orange-500" />
        <MetricBadge label="尖峰用電" value={metrics.electricity_load} unit="MW" icon={Zap} color="bg-yellow-50 text-yellow-600" />
